...
$mapImage = 'Dekarra.jpg';

$settlements = $pdo->query(
    'SELECT SettlementID, SettlementName, Notes, MapX, MapY
     FROM location_settlement
...
<body>
    ...
    <?php if ($userIsDM): ?>
        <p><em>DM mode: click the map to add a Settlement.</em></p>
    <?php endif; ?>

    <div id="map-wrap">
        <img src="<?= htmlspecialchars($mapImage) ?>" id="map-image" alt="Campaign map" style="<?= $userIsDM ? 'cursor: crosshair;' : '' ?>">

        <?php foreach ($Settlements as $loc): ?>
            <a class="pin"
               style="left: <?= htmlspecialchars($loc['MapX']) ?>%; top: <?= htmlspecialchars($loc['MapY']) ?>%;"
               href="Settlement.php?id=<?= (int) $loc['SettlementID'] ?>">
                <?= htmlspecialchars($loc['SettlementName']) ?>
            </a>
        <?php endforeach; ?>

        <?php if ($userIsDM): ?>
        <form id="new-pin-form" method="POST" action="save_Settlement.php">
